...
window.ChatBox = {
        ...
        <div class="center-panel">
            ...
            <div class="chat-header" v-if="currentConversation">
                ...
                <div class="chat-header-actions">
                    <button class="icon-btn" @click="showInfo" title="Beszélgetés infó">ℹ️</button>
                    <button class="icon-btn" @click="clearChat" title="Törlés" v-if="isAdmin">🗑️</button>
                </div>
            </div>
            ...
            <div class="chat-messages" ref="chatContainer" @scroll="handleScroll">
                ...
                <div v-for="(msg, index) in messages" :key="msg.id" 
                     class="message" 
                     :class="[msg.sender, msg.mood ? `mood-${msg.mood}` : '']"
                     :data-id="msg.id">
                    ...
                    <!-- Válasz gomb (quote) -->
                    <button class="quote-btn" @click="quoteMessage(msg)" v-if="msg.sender !== 'system'">
                        ↩️
                    </button>
                    
                    <!-- Token információ (ha van) -->
                    <div class="message-footer" v-if="msg.tokens">
                        <span class="token-count">🔤 {{ msg.tokens }} token</span>
                    </div>
                </div>
            ...
            <div class="chat-input-area">
                ...
                <div class="prompt-bar" v-if="prompts.length">
                    <select v-model="selectedPrompt" class="prompt-select" @change="loadPrompt">
                        <option value="">{{ gettext('chat.select_prompt') }}</option>
                ...
                <div class="input-container">
                    ...
                    <div v-if="quotedMessage" class="quote-preview">
                        <div class="quote-content">
                            <span class="quote-author">{{ quotedMessage.senderName }}:</span>
                    ...
                    <textarea 
                        ...
                        @input="handleTyping"
                        :placeholder="gettext('chat.type_message')"
                        :disabled="!connected"
                        class="chat-input"
                        rows="1"
                        ref="inputField"
                    ...
                    <div class="input-toolbar">
                        <button class="tool-btn" @click="uploadImage" title="Kép feltöltése">
                            📷
                        </button>
                        <button class="tool-btn" @click="openEmojiPicker" title="Hangulatjel">
                            😊
                        </button>
                        <button class="tool-btn" @click="clearInput" title="Törlés" v-if="inputMessage">
                            🗑️
                        </button>
                    </div>
                ...
                <div class="model-info" v-if="currentModel">
                    ...
                    <span v-if="tokenCount" class="token-info"> | 🔤 {{ tokenCount }} token</span>
                </div>
            </div>
        </div>
    `,
    ...
    setup() {
        ...
        const currentModel = Vue.computed(() => {
            ...
            return activeModel ? activeModel.name : window.gettext?.('chat.unknown_model') || 'Unknown';
        });
        ...
        const sendMessage = async () => {
            ...
            if (quotedMessage.value) {
                const quote = `> ${quotedMessage.value.senderName}: ${quotedMessage.value.text}\n\n`;
            ...
            reader.onload = (e) => {
                ...
                setTimeout(() => {
                    window.store?.setMessages(
                        window.store.messages.filter(m => m.id !== tempId)
                    );
                }, 1000);
            };
        ...
        const formatMessage = (text) => {
            ...
            if (formatted.match(/\.(jpg|jpeg|png|gif|webp)(\?.*)?$/i)) {
                formatted = formatted.replace(
                    ...
                    '<img src="$1" alt="Image" class="chat-image" loading="lazy">'
                );
            }
        ...
        const clearChat = () => {
            ...
            const confirmMsg = window.gettext?.('chat.confirm_clear') || 'Clear this conversation?';
