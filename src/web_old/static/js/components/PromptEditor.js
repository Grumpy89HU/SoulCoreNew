...
window.PromptEditor = {
            ...
            <div class="prompt-header">
                ...
                <div class="header-actions">
                    ...
                    <button class="btn-primary" @click="createNew" v-if="isAdmin" :disabled="loading">
                        <span v-if="!loading">+ {{ t('prompts.new') }}</span>
                    ...
                    <div class="prompt-list">
                        <div v-for="prompt in filteredAndSortedPrompts" :key="prompt.id" 
                            ...
                            <div class="prompt-stats" v-if="prompt.usage_count">
                                <span class="stat">📊 {{ prompt.usage_count }} {{ t('prompts.uses') }}</span>
                            </div>
                        </div>
                    ...
                    <div class="edit-content">
                        <div class="content-header">
                            ...
                            <div class="content-stats">
                                <span class="stat">📏 {{ contentLength }} {{ t('prompts.chars') }}</span>
                                <span class="stat">🔤 {{ estimatedTokens }} {{ t('prompts.tokens') }}</span>
                            </div>
                        </div>
    ...
    setup() {
        // ====================================================================
        // BIZTONSÁGOS GETTEXT FÜGGVÉNY
        // ====================================================================
        
        // Ez a függvény mindig elérhető, még akkor is, ha a window.gettext még nem töltött be
        const t = (key, params = {}) => {
            // Ha van window.gettext, használjuk
            if (window.gettext) {
        ...
        const availableVariables = {
            'text': t('prompts.var_text'),
            'user': t('prompts.var_user'),
            'date': t('prompts.var_date'),
            'time': t('prompts.var_time'),
            'context': t('prompts.var_context'),
            'history': t('prompts.var_history')
        };
        ...
        const formatRelativeTime = (dateStr) => {
            ...
            try {
                ...
                if (diffMins < 1440) return `${Math.floor(diffMins / 60)} ${t('time.hours_ago')}`;
        ...
        const testWithInput = () => {
            ...
            alert(t('prompts.test_not_implemented'));
        };
                ...
                if (window.api) {
                    ...
                    if (result.success) {
                        ...
                        if (window.showNotification) {
                            window.showNotification(t('prompts.saved'), 'success');
                        }
                    }
                }
            } catch (error) {
                alert(t('prompts.save_error', { error: error.message }));
            } finally {
                saving.value = false;
            }
        };
        ...
        const deletePrompt = async () => {
            ...
            const message = t('prompts.confirm_delete', { name: selectedPrompt.value.name });
            ...
            try {
                if (window.api) {
                    ...
                    // Értesítés
                    if (window.showNotification) {
                        window.showNotification(t('prompts.deleted'), 'success');
                    }
                }
            } catch (error) {
                alert(t('prompts.delete_error', { error: error.message }));
            } finally {
                saving.value = false;
            }
        };
            ...
            promptVersions.value = [
                {
                    ...
                    comment: t('prompts.version_improved')
                },
                {
                    id: 2,
                    version: 2,
                    created_at: Date.now() - 172800000,
                    comment: t('prompts.version_initial')
                }
            ];
        };
        ...
        // ====================================================================
        // RETURN - itt adjuk vissza a t függvényt is!
        // ====================================================================
        
        return {
            // EZ A LEGFONTOSABB: a t függvényt vissza kell adni, hogy a template lássa!
            t,
...
console.log('✅ PromptEditor betöltve');