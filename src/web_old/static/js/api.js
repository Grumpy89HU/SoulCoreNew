...
window.api = {
    ...
    async getAuditLog(limit = 100) {  // <-- Alapértelmezett 100
		const qs = this._buildQueryString({ limit: Math.min(limit, 500) });  // Max 500
    ...
    async getMetrics(params = {}) {
		// params lehet: { period: 'day', limit: 100 }
		// vagy egyszerűen: 'day' (string)
		
		let period = 'hour';
		let limit = 100;
		
		if (typeof params === 'string') {
			// Ha string, akkor az a period
			period = params;
		} else {
			// Ha object, akkor kibontjuk
